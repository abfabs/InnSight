...
class SentimentETL:
    ...
    def process_reviews(self):
        ...
        # PROCESSED listings to filter valid listings only
        ...
        # Save review-level sentiment with city
        output_file = self.processed_path / 'reviews_sentiment.csv'
        reviews[['listing_id', 'id', 'date', 'sentiment', 'sentiment_category']].to_csv(output_file, index=False)
        print(f"✓ Saved: {output_file} ({len(reviews)} reviews)")
        
        return reviews
    
    def create_listing_sentiment(self, reviews):
        print(f"Aggregating sentiment per listing...")
    ...
    def create_neighborhood_sentiment(self, reviews):
        print(f"Creating neighborhood sentiment stats...")
        
        # PROCESSED listings_clean.csv (clean neighborhoods)
        listings = pd.read_csv(self.processed_path / 'listings_clean.csv')
        
        reviews_with_hood = reviews.merge(
            listings[['listing_id', 'neighbourhood']], 
            on='listing_id',
            how='inner'
        )
        
        print(f"  Reviews with neighborhood: {len(reviews_with_hood)}")
        
        # Aggregate by neighborhood
        neighborhood_sentiment = reviews_with_hood.groupby('neighbourhood')['sentiment'].agg([
            'mean', 'median', 'std', 'count'
        ]).reset_index()
        
        neighborhood_sentiment.columns = [
            'neighbourhood', 'sentiment_mean', 'sentiment_median', 
        ...
        sentiment_dist = reviews_with_hood.groupby(['neighbourhood', 'sentiment_category']).size().unstack(fill_value=0)
        sentiment_dist = sentiment_dist.div(sentiment_dist.sum(axis=1), axis=0) * 100
        sentiment_dist = sentiment_dist.reset_index()
        
        neighborhood_sentiment = neighborhood_sentiment.merge(sentiment_dist, on='neighbourhood', how='left')
    ...
    def create_listings_map(self):
        print(f"Creating map data with sentiment...")
    ...
    def run(self):
        ...
        listing_sentiment = self.create_listing_sentiment(reviews)
        neighborhood_sentiment = self.create_neighborhood_sentiment(reviews)
        map_data = self.create_listings_map()
